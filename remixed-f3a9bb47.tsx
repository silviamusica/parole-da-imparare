...
import { Upload, Shuffle, Eye, EyeOff, ChevronLeft, ChevronRight, ChevronDown, Check, X, Brain, Zap, Star, RotateCcw, Trophy, Target, Clock, Flame, BookOpen, Sparkles, ArrowRight, Heart, HelpCircle } from 'lucide-react';

// Parser CSV semplice - STRUTTURA: parola, accento, definizione, etimologia, esempio
const parseCSV = (text) => {
...
export default function LessicoGame() {
  ...
  // Genera hint tipo impiccato - PROGRESSIVO
  const generateHint = (word, level) => {
    const len = word.length;
    const revealed = new Set();
    
    // Livello 0: prima e ultima lettera
    if (level === 0) {
      revealed.add(0);
      revealed.add(len - 1);
    } else {
      // Livelli successivi: aggiungi lettere progressivamente
      revealed.add(0);
      revealed.add(len - 1);
      
      // Aggiungi lettere in posizioni strategiche
      const positions = [];
      
      // Aggiungi posizioni intermedie
      ...
      // Rivela tante lettere quanti sono i livelli di aiuto
      for (let i = 0; i < Math.min(level, positions.length); i++) {
        revealed.add(positions[i]);
      }
    }
    
    // Calcola percentuale rivelata
    const percentRevealed = (revealed.size / len) * 100;
    
    // Se oltre l'80%, hai perso
    if (percentRevealed > 80) {
      return { hint: word, lost: true, percent: 100 };
    }
    
    // Costruisci l'hint
    const hint = word.split('').map((char, idx) => 
      revealed.has(idx) ? char : '_'
    ).join('');
  ...
  const startGame = (mode, limit) => {
    ...
    const shuffled = shuffleArray(words).slice(0, actualLimit || words.length);
    setShuffledWords(shuffled);
    
    if (mode === 'quiz' || mode === 'speedQuiz') {
      setQuizOptions(generateQuizOptions(shuffled[0], words));
      if (mode === 'speedQuiz') {
        setTimeLeft(15);
  ...
  const nextWord = () => {
    ...
    if (currentIndex < Math.min(shuffledWords.length, limit) - 1) {
      ...
      if (gameMode === 'quiz' || gameMode === 'speedQuiz') {
        setQuizOptions(generateQuizOptions(shuffledWords[newIndex], words));
  ...
  const handleCorrectAnswer = () => {
    setIsCorrect(true);
    setScore(score + (gameMode === 'speedQuiz' ? timeLeft * 10 : 100));
    setStreak(streak + 1);
  ...
  const handleWrongAnswer = (selectedWrongWord = null) => {
    setIsCorrect(false);
    setStreak(0);
    ...
    setShowCorrectAnswer(correctWord);
    ...
    setWordsToReview(prev => {
      ...
      if (!exists) {
        return [...prev, { ...correctWord, wrongAnswer: selectedWrongWord?.term || null }];
      }
      return prev;
    });
  };
  ...
  const formatWordsForExport = (format) => {
    if (format === 'text') {
      ...
      const header = 'Parola,Accento,Definizione,Etimologia,Esempio,Da_Rivedere\n';
      const rows = wordsToReview.map(w => 
        `"${w.term}","${w.accent || ''}","${w.definition}","${w.etymology || ''}","${w.example || ''}","SI"`
      ).join('\n');
      return header + rows;
      ...
      const rows = words.map(w => 
        `"${w.term}","${w.accent || ''}","${w.definition}","${w.etymology || ''}","${w.example || ''}","${reviewTerms.has(w.term) ? 'SI' : 'NO'}"`
      ).join('\n');
      return header + rows;
    } else if (format === 'list') {
      ...
      <div className="bg-slate-800/40 backdrop-blur-lg rounded-3xl p-8 max-w-md w-full text-center border border-slate-700/50">
        ...
        <label className="block cursor-pointer group">
          <div className="border-2 border-dashed border-slate-600 rounded-2xl p-8 transition-all group-hover:border-cyan-700 group-hover:bg-slate-700/20">
            ...
            <p className="text-slate-500 text-sm">Formato: parola, accento, definizione, etimologia, esempio</p>
          </div>
      ...
      <div className="max-w-2xl mx-auto pt-8">
        <div className="text-center mb-8">
          ...
          <div className="flex justify-center gap-4 mt-4">
            <div className="bg-cyan-900/30 px-4 py-2 rounded-full border border-cyan-800/50">
              <span className="text-cyan-400 font-bold">{score}</span>
              <span className="text-slate-400 text-sm ml-1">punti totali</span>
            </div>
            <div className="bg-sky-900/30 px-4 py-2 rounded-full border border-sky-800/50">
              <span className="text-sky-400 font-bold">{maxStreak}</span>
              <span className="text-slate-400 text-sm ml-1">max streak</span>
            </div>
          </div>
        </div>
  ...
  const GameModeCard = ({ icon, title, description, color, onClick }) => (
    <button
      onClick={onClick}
      className={`bg-gradient-to-r ${color} p-6 rounded-2xl text-left flex items-center gap-4 transform transition-all hover:scale-102 hover:shadow-lg border border-slate-700/50 active:scale-98 w-full`}
    >
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-zinc-900 flex items-center justify-center p-4">
        <div className="bg-slate-800/40 backdrop-blur-lg rounded-3xl p-8 max-w-md w-full text-center border border-slate-700/50">
          ...
          <div className="grid gap-4">
            ...
            {words.length > 50 && (
              <button
                onClick={() => startGame(pendingMode, words.length)}
              ...
              >
                ...
                <span className="text-slate-300">({words.length} parole)</span>
              </button>
            )}
          </div>
  ...
  const GameHeader = () => (
    <div className="flex items-center justify-between mb-6">
      ...
      <div className="flex items-center gap-4">
        ...
        {gameMode !== 'match' && gameMode !== 'flashcard' && (
          <>
            <div className="bg-cyan-900/30 px-3 py-1 rounded-full flex items-center gap-1 border border-cyan-800/50">
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-zinc-900 p-4">
        <div className="max-w-lg mx-auto pt-4">
          ...
          <div 
            ...
            className="bg-slate-800/40 backdrop-blur-lg rounded-3xl p-8 min-h-[300px] flex flex-col justify-center cursor-pointer border border-slate-700/50 transform transition-all hover:scale-102"
          >
            {!showAnswer ? (
              <>
            ...
            <div className="flex justify-center gap-4 w-full">
              <button
                onClick={() => { 
                  const word = shuffledWords[currentIndex];
                  setWordsToReview(prev => {
                    const exists = prev.some(w => w.term === word.term);
                    if (!exists) {
                      return [...prev, { ...word, wrongAnswer: null }];
                    }
                    return prev;
                  });
                  setShowAnswer(false); 
              ...
              <button
                onClick={() => { setShowAnswer(false); nextWord(); }}
                    ...
                    selectedAnswer === null
                      ...
                      : selectedAnswer === option.term
                        ? option.term === word.term
                          ? 'bg-cyan-900/50 text-slate-100 scale-102 border border-cyan-800/50'
      ...
      <div className="bg-slate-800/40 backdrop-blur-lg rounded-3xl p-8 max-w-md w-full text-center border border-slate-700/50">
        ...
        <div className="grid grid-cols-2 gap-4 my-8">
          <div className="bg-slate-800/30 rounded-xl p-4 border border-slate-700/50">
            ...
            <p className="text-slate-400 text-sm">Punti</p>
          </div>
          <div className="bg-slate-800/30 rounded-xl p-4 border border-slate-700/50">
